...
var Se = {
  ...
  setup(B, { emit: x }) {
    const v = x, u = B, w = useSlots(), a = ref([]), k = computed(() => a.value.reduce((e, n) => (e[~~n.id] = n) && e, {})), d = computed(() => a.value.length), h2 = ref(null), S = ref(false), c = ref({
      mouseDown: false,
      dragging: false,
      activeSplitter: null,
      cursorOffset: 0
      ...
      y(), c.value.mouseDown = true, c.value.activeSplitter = n;
    }, r = (e) => {
      ...
      c.value.dragging && v("resized", a.value.map((e) => ({ min: e.min, max: e.max, size: e.size }))), c.value.mouseDown = false, setTimeout(() => {
        c.value.dragging = false, D();
      }, 100);
    }, O = (e, n) => {
      ...
      const n = h2.value.getBoundingClientRect(), { clientX: t, clientY: i } = "ontouchstart" in window && e.touches ? e.touches[0] : e;
      return {
      ...
      const n = h2.value[u.horizontal ? "clientHeight" : "clientWidth"];
...
var Pe = {
  ...
  setup(B) {
    var R;
    const x = B, v = inject("requestUpdate"), u = inject("onPaneAdd"), w = inject("horizontal"), a = inject("onPaneRemove"), k = inject("onPaneClick"), d = (R = getCurrentInstance()) == null ? void 0 : R.uid, h2 = inject("indexedPanes"), S = computed(() => h2.value[d]), c = ref(null), f = computed(() => {
